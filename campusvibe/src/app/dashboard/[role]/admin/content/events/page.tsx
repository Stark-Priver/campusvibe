...
export default async function AdminContentEventsPage({ params, searchParams }: Props & { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const { data: events } = await supabase
    .from("events")
    .select("id, title, slug, category, date, location, university, is_published, updated_at")
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id} className="border-b border-slate-50 align-top last:border-0">
                ...
                <td className="py-2.5 pr-3 text-slate-700">{item.date}</td>
                <td className="py-2.5 pr-3 text-slate-700">{item.location ?? "-"}</td>
                <td className="py-2.5 pr-3">
                ...
                <td className="py-2.5">
                  <form action={updateEventItem.bind(null, role)} className="grid grid-cols-1 md:grid-cols-6 gap-2 mb-2">
                    ...
                    <input name="date" type="date" defaultValue={item.date} className="rounded border border-slate-300 px-2 py-1" />
                    <input name="location" defaultValue={item.location ?? ""} className="rounded border border-slate-300 px-2 py-1" />
                  ...
                  <form action={deleteEventItem.bind(null, role)}>
                    <input type="hidden" name="id" value={item.id} />
