            ...
            <AnimatedSection key={item.id} delay={0.07 * i}>
              ...
              >
                <div className="relative aspect-video bg-gradient-to-br from-gray-800 to-gray-900">
                  ...
                  {item.duration && (
                    <span className="absolute bottom-2 right-2 text-white text-[10px] font-body bg-black/60 px-1.5 py-0.5 rounded">
                      {item.duration}
                    </span>
                  )}
                  <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-black/50 text-white text-[10px] font-section font-semibold capitalize">
                    {item.type}
                  </span>
                </div>
                <div className="flex flex-col flex-1 p-4">
                  <p className="text-[10px] text-muted font-body mb-1.5">
                    {item.channel} · {item.views_count.toLocaleString()} views
                  </p>
